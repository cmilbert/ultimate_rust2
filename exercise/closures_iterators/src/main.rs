fn main() {
    ...
    let pairs_challenge = vec![(0, 1), (2, 3), (4, 5)];
    for pair in pairs_challenge {
    ...
    let numbers_challenge = vec![1, 2, 3, 4];
    let numbers_challenge: Vec<i32> = numbers_challenge.into_iter().map(|x| x * 3).collect();
    println!("{:?}", numbers_challenge); // should print [3, 6, 9, 12]
}
